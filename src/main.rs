...
fn try_main() -> Result<()> {
    ...
    let sdl_context = sdl2::init().map_err(|err| Error::sdl2(err))?;
    let video_subsystem = sdl_context.video().map_err(|err| Error::sdl2(err))?;
    ...
    let _ctx = window.gl_create_context().map_err(|e| Error::sdl2(e))?;
    ...
    let mut event_pump = sdl_context.event_pump().map_err(|e| Error::sdl2(e))?;
    gst::init()?;
...
fn pretty_error(err: &failure::Error) -> String {
    ...
    pretty.push_str("Correct the error and save your shader to continue\n");
